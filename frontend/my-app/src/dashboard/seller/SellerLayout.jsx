...
import { MdGridView, MdStorefront, MdLogout, MdDeleteOutline, MdBrush } from "react-icons/md";
...
const SellerLayout = () => {
  ...
  const deleteAccount = async () => {
    if (!window.confirm("CRITICAL: Are you sure? This will permanently erase your gallery and seller history.")) {
      return;
    }
    try {
    ...
    <div className="flex min-h-screen bg-[#FCFCFD]">
      ...
      <aside className="w-64 bg-white border-r border-gray-100 fixed h-full z-20 transition-all duration-300">
        <div className="px-8 py-10">
          ...
          <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-1">Seller Portal</p>
        </div>

        <nav className="mt-4 flex flex-col h-[calc(100%-180px)] justify-between">
          {/* Main Nav */}
          <div className="space-y-1">
            <NavItem to="/seller" icon={<AiOutlineHome size={18} />} label="Overview" />
            <NavItem to="/seller/products" icon={<MdGridView size={18} />} label="Collection" />
          </div>
          ...
          <div className="px-10 py-5 flex items-center justify-between">
            ...
            <div className="flex items-center gap-6">
              <button
                ...
                className="group flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-black border-2 border-black px-5 py-2 hover:bg-black hover:text-white transition-all duration-300"
              >
...
const NavItem = ({ to, icon, label }) => (
  <NavLink
    ...
    className={({ isActive }) =>
      `group relative w-full flex items-center gap-4 px-8 py-4 text-[11px] font-black uppercase tracking-[0.15em] transition-all duration-300
      ${isActive 
